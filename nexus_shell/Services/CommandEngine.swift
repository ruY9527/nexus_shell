...
//  nexus_shell
//
//  Unified command engine for both simulated and real SSH connections
//

import Foundation

/// 命令引擎协议 - 统一命令模拟逻辑
protocol CommandEngine {
...
class DefaultCommandEngine: CommandEngine {
    ...
    func execute(_ command: String) -> String {
        ...
        // 处理 cd 命令（改变当前目录）
        if let cdResult = handleCDCommand(trimmed) {
            return cdResult
        }
