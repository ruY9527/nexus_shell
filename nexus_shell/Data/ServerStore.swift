...
class ServerStore: ObservableObject {
    ...
    func addSampleData() {
        // 先创建示例文件夹
        let folderStore = FolderStore.shared
        
        let companyA = folderStore.createFolder(
            name: "Company A",
            color: .blue,
            icon: .building,
            description: "Main company servers"
        )
        
        let companyB = folderStore.createFolder(
            name: "Company B",
            color: .green,
            icon: .network,
            description: "Client company servers"
        )
        
        // 添加服务器到不同文件夹
        let sampleServers = [
            Server(
                folderId: companyA.id,
            ...
            Server(
                folderId: companyB.id,
                name: "Production Server",
                host: "192.168.1.100",  // 相同内网IP
                port: 22,
                username: "admin",
                tags: ["production"],
                notes: "Company B production"
            ),
            Server(
                folderId: nil,  // 根目录
                name: "Public Server",
                ...
                username: "user",
                tags: ["public"],
                notes: "Public access server"
            )
        ]
        
        // 设置示例状态
        sampleServers[0].status = .online
        sampleServers[0].cpuUsage = 45.5
        sampleServers[0].memoryUsage = 62.3
        
        sampleServers[1].status = .online
        sampleServers[1].cpuUsage = 12.8
        sampleServers[1].memoryUsage = 35.1
        
        sampleServers[2].status = .warning
        sampleServers[2].cpuUsage = 89.2
        sampleServers[2].memoryUsage = 91.5
